...
const newItem = (): MenuItem => ({
  ...
  description: null,
  contentType: "text",
  content: { type: "text", title: "Menu Baru", body: "" },
  mediaId: null,
  active: true,
  sortOrder: 99,
});
...
export function MenuPage() {
  ...
  const refresh = useCallback(async () => {
    const [menus, mediaResponse] = await Promise.all([
      apiClient.get<{ items: MenuItem[] }>("/api/v1/admin/menu-items"),
      apiClient.get<{ items: MediaOption[] }>("/api/v1/admin/media"),
    ]);
    setItems(menus.items);
  ...
  async function save(item: MenuItem) {
    if (creating) await apiClient.post(`/api/v1/admin/menu-items`, item, csrfToken);
    else await apiClient.put(`/api/v1/admin/menu-items/${item.id}`, item, csrfToken);
    setCreating(false);
    setEditing(null);
    await refresh();
  }
  ...
  async function remove() {
    if (!deleting) return;
    await apiClient.delete(`/api/v1/admin/menu-items/${deleting.id}`, csrfToken);
    setDeleting(null);
    await refresh();
  }
  ...
  return (
    <section className="cms-page">
      <header className="cms-page-header">
        <div><h1 className="cms-page-title">Menu & Konten</h1><p className="cms-page-description">Kelola layanan dan tipe konten yang tampil di kiosk.</p></div>
        <Button variant="primary" onClick={() => { setCreating(true); setEditing(newItem()); }}>Tambah Menu</Button>
      </header>
      ...
      <LayerDialog.Root open={Boolean(deleting)} onOpenChange={(open) => !open && setDeleting(null)}>
        <LayerDialog.Content closeLabel="Tutup">
          ...
          <LayerDialog.Actions dismissLabel="Batal"><LayerDialog.Actions.Primary variant="destructive" onClick={() => void remove()}>Hapus</LayerDialog.Actions.Primary></LayerDialog.Actions>
        </LayerDialog.Content>
      </LayerDialog.Root>
    </section>
  );
