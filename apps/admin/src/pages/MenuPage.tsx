...
export function MenuPage() {
  ...
  return (
    <section className="cms-page">
      ...
      <LayerDialog.Root open={Boolean(deleting)} onOpenChange={(open: boolean) => !open && setDeleting(null)}>
        <LayerDialog.Content closeLabel="Tutup"><LayerDialog.Title>Hapus menu?</LayerDialog.Title><LayerDialog.Description>{deleting?.name}</LayerDialog.Description><LayerDialog.Body>Penghapusan mengubah draft. Kiosk tidak berubah sampai konten dipublish.</LayerDialog.Body><LayerDialog.Actions dismissLabel="Batal"><LayerDialog.Actions.Primary variant="destructive" onClick={() => void remove()}>Hapus</LayerDialog.Actions.Primary></LayerDialog.Actions></LayerDialog.Content>
      </LayerDialog.Root>
    </section>
  );
}
