...
export function PublishPanel({ currentVersion, hasChanges, onPublish }: { currentVersion: number | null; hasChanges: boolean; onPublish(): Promise<void> }) {
  ...
  return (
    <LayerDialog.Root open={open} onOpenChange={setOpen} dismissDisabled={publishing}>
      <LayerDialog.Trigger render={(props) => (
        <Button {...props} variant="primary" disabled={!hasChanges}>Publish ke Kiosk</Button>
      )} />
      <LayerDialog.Content closeLabel="Tutup">
        ...
        <LayerDialog.Body>
          <p>Kiosk baru mengaktifkan versi ini setelah seluruh file yang dibutuhkan berhasil diunduh dan diverifikasi.</p>
        </LayerDialog.Body>
        <LayerDialog.Actions dismissLabel="Batal">
          <LayerDialog.Actions.Primary loading={publishing} onClick={() => void publish()}>
            Publish sekarang
